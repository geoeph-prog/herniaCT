...
/**
 * The DefaultDateCellRenderer class provides custom rendering for date cells in a DatePicker
 * component. It defines the visual appearance and behavior of date cells, including selection
 * highlighting, painting of the current date, focus effects, and rendering dates between a selected
 * range.
 *
 * @author Raven Laing
...
public class DefaultDateCellRenderer {
  ...
  protected void paintDateSelection(
      Graphics2D g2,
      DatePicker datePicker,
      ButtonDate component,
      boolean isSelected,
  ...
  protected void paintBetweenDateSelection(
      Graphics2D g2,
      DatePicker datePicker,
      ButtonDate component,
      SingleDate date,
  ...
  protected void paintCurrentDate(
      Graphics2D g2,
      DatePicker datePicker,
      ButtonDate component,
      SingleDate date,
    ...
    Area area =
        new Area(FlatUIUtils.createComponentRectangle(rec.x, rec.y, rec.width, rec.height, arc));
    if (isSelected) {
      float s = UIScale.scale(fw + 1f);
      area.subtract(
          new Area(
              FlatUIUtils.createComponentRectangle(
                  rec.x + s, rec.x + s, rec.width - s * 2f, rec.height - s * 2f, arc - s)));
    } else {
      float s = UIScale.scale(fw);
      area.subtract(
          new Area(
              FlatUIUtils.createComponentRectangle(
                  rec.x + s, rec.y + s, rec.width - s * 2f, rec.height - s * 2f, arc - s)));
    }
  ...
  protected Shape createShape(
      float x, float y, float width, float size, float arc, boolean right, boolean add) {
    Area area;
    if (right) {
  ...
  protected Color getSelectionColor(
      DatePicker datePicker, ButtonDate component, boolean isSelected) {
